(function () {
  ...
  const projectUrl = locationHref.substring(0, locationHref.lastIndexOf("/"));
  ...
  var links = (text = "") => {
    ...
    for (var i = 0; i < aTags.length; i++) {
      ...
      // scrapboxのリンクが含まれる場合は削除
      // var linkMarkdown = `[${aHtml}](${aHref})`;
      var linkMarkdown = aHref.match(projectUrl) ? "" : `${aHref}`;

      // 画像対応
      var img = a.querySelector("img");
      if (img !== null) {
        // 画像もscrapboxかgyazoのリンクなので削除
